...
open class UUCircleView(context: Context, attrs: AttributeSet?, defStyle: Int): View(context, attrs, defStyle)
{
    ...
    init
    {
        ...
        attrs?.let()
        {
            val a = context.obtainStyledAttributes(attrs, R.styleable.UUCircleView)

            val borderWidth = a.getDimensionPixelSize(R.styleable.UUCircleView_uuBorderWidth, -1)
            if (borderWidth != -1)
            {
                borderPaint.strokeWidth = borderWidth.toFloat()
            }

            val borderColor = a.getColor(R.styleable.UUCircleView_uuBorderColor, -1)
            if (borderColor != -1)
            {
                borderPaint.color = borderColor
            }

            val fillColor = a.getColor(R.styleable.UUCircleView_uuFillColor, -1)
            if (fillColor != -1)
            {
