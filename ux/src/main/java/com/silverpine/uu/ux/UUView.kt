...
@BindingAdapter(value = ["bind:uuAlpha", "bind:uuAlphaDuration"], requireAll = false)
fun uuBindAlpha(view: View, alpha: Float?, duration: Long = 200L)
{
    alpha?.let()
    {
        if (view.alpha != it)
        {
            view.uuFadeAlpha(it, duration)
        }
    }
}